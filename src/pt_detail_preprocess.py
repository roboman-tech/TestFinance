...
if __name__ == "__main__":
    # Create directories
    training_data_dir = 'training_data/pt_detail'
    test_data_dir = 'test_data/pt_detail'
    os.makedirs(training_data_dir, exist_ok=True)
    os.makedirs(test_data_dir, exist_ok=True)

    # Load data
    df = pd.read_parquet('input_data/price_target_detail_all_data.parquet')# .query("permno == 14593")
    ...
    df = df.dropna(subset=['fwd_ret_1y_excl_div', 'value'], how='any')
    ...
    # fillna with median
    df_processed[f_cols] = df_processed[f_cols].fillna(df_processed[f_cols].median())
